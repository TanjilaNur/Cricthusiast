...
class FeaturedMatchesCollectionViewCell: UICollectionViewCell {
    ...
    func setValues(model: MatchCellModel) {
        ...
        if model.tag == 1 {
            let formattedTime = formatTime(from: model.matchNote)
            noteLabel.text = formattedTime
    ...
    func updateCountDown(time: String) {
        ...
        if noteLabel.text?.first == "-" {
            noteLabel.text = "Match has already started!"
        }
