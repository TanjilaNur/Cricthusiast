...
//        leagueImageView.layer.cornerRadius = leagueImageView.frame.height / 2
    ...
    func setModelValues(model: MatchInfoTVCellModel){
        ...
        leagueImageView.sd_setImage(with: URL(string: model.leagueImgUrl), placeholderImage: UIImage(named: "league"), options: .progressiveLoad)
    }
    ...
    func setModelValuesForVMom(model: MatchInfoTVCellModel){
        leagueImageView.sd_setImage(with: URL(string: model.manOfMatchImage), placeholderImage: UIImage(named: "cricket") , options: .progressiveLoad)
        leagueName.text = "MoM: \(model.manOfMatch)"
        season.text = "Position: \(model.manOfmatchPosition)"
        round.text = ""
    }
